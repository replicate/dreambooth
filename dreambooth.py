...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # Don't save checkpoint and don't generate validations
                # if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        ...
        if args.generate_images is not None:
            ...
            with torch.autocast("cuda"), torch.inference_mode():
                for info in tqdm(args.generate_images, desc="Generating samples"):
                    ...
                    num_outputs = int(input.get("num_outputs", 1))
                    disable_safety_check = bool(
                        input.get("disable_safety_check", False)
                    )
                    num_inference_steps = int(input.get("num_inference_steps", 50))
                    guidance_scale = float(input.get("guidance_scale", 7.5))
                    scheduler = input.get("scheduler", "DDIM")
                    seed = input.get("seed")
                    if seed is None:
                    ...
                    for i, image in enumerate(output.images):
                        if (
                            output.nsfw_content_detected
                            and output.nsfw_content_detected[i]
                        ):
                            print("skipping nsfw detected for", inputs)
                            continue
                        image.save(
                            os.path.join(args.generate_images_dir, f"{name}-{i}.png")
                        )
        ...
        if args.push_to_hub:
            print(f"push_to_hub not implemented")
